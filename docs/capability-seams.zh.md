...
flowchart LR
  ...
  svc_desktopStartup["ctx.desktopStartup<br/>Desktop invocation acceptance"]
  svc_desktopRuntime["ctx.desktopRuntime<br/>Desktop Host surface marker"]
  ...
  pkg_desktop_app --> svc_desktopStartup
  ...
  svc_desktopStartup --> pkg_connection
  svc_desktopStartup --> pkg_desktop_app
  svc_desktopStartup --> pkg_modules
...
| `ctx.desktopStartup` | `bundle` | [`desktop-app`](../packages/bundle/desktop-app) | - | [`desktop-app`](../packages/bundle/desktop-app), `modules`, `connection` | - | desktop profile 的仅表示“调用已被接受”的标记：`--help` 时不提供它，客户端图各行保持挂起，且没有绑定 flag。 |
| `ctx.desktopRuntime` | `bundle` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | 标记已结算的 desktop Host，不含绑定地址；Electron 壳读取此 Host 已经组合好的客户端图。 |
