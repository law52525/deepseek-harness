...
P0 提供 IPC 载体。[P1](./2026-08-14-desktop-profile-host.zh.md) 提供带客户端图、无 HTTP 的 Host profile。还没有人创建窗口、加载 `dsh-web-frontend` dist、注入 `window.__DSH_BOOT__`，或拉取插件 bundle。Web 壳内核 [`AppWebEntry`](../../../../packages/client/web/README.zh.md) 期望该清单在 `window` 上，并通过 `<script src="/plugins/…">` 加载插件。`file://` 的 origin 是 `'null'`，经典 script 标签无法使用 Host 的 `/plugins` 路由（根本没有这些路由），而 `WebApiClient` 会对这个空 origin 调用 `globalThis.fetch`。

`BootSeams.loadBundle` 已经在 jsdom 测试里替换脚本到达。connection 客户端的 `apply` 在未设置 `?fixture` 时构造的是 `WebApiClient`。

## Decision

**`apps/desktop`**（`@deepseek-ai/dsh-desktop`）是 Electron 应用组装，不是能力包。`pnpm run dev:desktop` 先跑 `pnpm run build` 再运行 `electron .`。生产打包属于 [P4](../process/2026-08-14-desktop-installer-packaging.zh.md)。

### 进程模型

Electron main 用系统 Node 孵化子进程（`npm_node_execpath`、`NODE_BINARY` 或 `PATH` 上的 `node`——当 `process.execPath` 是 Electron 时绝不使用它，也绝不使用 `ELECTRON_RUN_AS_NODE`），`stdio: ['ignore', 'inherit', 'inherit', 'ipc']`，且 `windowsHide: true`。argv 为 `dsh --profile desktop`（存在 `apps/cli/src/bin.ts` 时用 source `tsx` 启动器，否则用构建出的 bin）。子进程在 `@deepseek-ai/dsh-desktop-app/ipc-host` 内运行 [`HostIpcGateway`](./2026-08-14-desktop-ipc-carrier.zh.md)。main 是哑转发器：renderer ↔ preload ↔ main ↔ child。RPC payload 在 `rpc` 通道上保持不透明 JSON；main 从不解析 body。

子进程在 `loader.await()` 之后（没有 Loader 时立即）发送 `host-ready`。终端里的 `dsh desktop` 没有父进程 IPC 通道，因此 `ipc-host` 为空操作，P1 的 CLI 行为不变。

退出时，main 中止进行中的 IPC handler，对子进程 SIGTERM，等待，必要时再 SIGKILL。子进程崩溃则加载错误页；窗口不会继续挂在已死的端口上。打包后的构建会检查 GitHub Releases 做整应用更新（[P5](../process/2026-08-14-desktop-installer-release-ci.zh.md)）；缺少 feed 不会退出应用。
...
Session Header 的 `Session log` 控件仍对页面 origin 发 `HEAD` 再跟 `<a download>`。在 `dsh://app` 下该 URL 是 `/api/session.export`，不是前端 dist 文件。`dsh:` handler 只把该路径上的 GET/HEAD 转给 Host 子进程，作为 control 文档（`sessionId` 与 `includeDescendants` 查询字段，从不是任意 URL）。子进程跑现有的 `toFetchHandler` 下载；GET 正文写入临时 ZIP，路径经 control 返回。main 读入该文件作为 protocol Response 并删除它。JSON RPC 保持仅 UTF-8（[P0](./2026-08-14-desktop-ipc-carrier.zh.md)）；ZIP 不走 `unary-response`。该路径上的其他方法应答 405。缺少 Host 子进程应答 503。
...
渲染进程复用 `dsh-web-frontend` dist。main 注册特权 `dsh:` scheme（`standard`、`secure`、`supportFetchAPI`），并在 `dsh://app/` 提供该 dist，这样 Vite 的 `/assets/…` URL 无需改 web 的 `base`。同一 handler 按上文从 Host 子进程应答 `GET`/`HEAD` `/api/session.export`。`apps/web/src/main.ts` 根据 `__DSH_DESKTOP__` 分支并调用 `runDesktopBoot`。该函数从 Host 图设置 `window.__DSH_BOOT__`，在不导入 `ui-theme` 的情况下应用插件前主题，eval `bootstrapFacadeScript()` 以 queue 模式安装 `window.__ModuleLoader__`（与 web Host 注入 index.html 的同一段内联脚本；打包后的 Vite `index.html` 没有 Host HTML 注入），经 IPC 加载 `PARSER_PRELOAD_IDS` bundle（先 `@deepseek-ai/dsh-client-modules`，再 `@deepseek-ai/dsh-client-runtime`）使它们的 factory 在 `create()` 之前登记，然后运行 `new AppWebEntry(el, { loadBundle })`。`loadBundle` 把 `/plugins/<id>/client.js?rev=…` 映射到 `readPlugin(id)`，并以与 jsdom assembled boot 相同的方式求值 factory。省略 facade 或这些 parser-preload 会使 `AppWebEntry.run()` 抛出 `web boot: window.__ModuleLoader__ bootstrap facade is missing`。
...
原生目录选择器保持 Node 的 `-native` 配对。[P3](./2026-08-14-desktop-native-shell-capabilities.zh.md) 经子进程 IPC 证明 `host.pickDirectory` 与 `host.openPath`。子进程孵化使用 `windowsHide: true`，以免 Windows 打开 `node.exe` 控制台；Win32 选择器 worker 本身已经使用 `windowsHide: true`。

## Related

- [桌面产品](./2026-08-14-desktop-installer-product.zh.md)
- [P0 IPC 载体](./2026-08-14-desktop-ipc-carrier.zh.md)
- [P1 desktop profile Host](./2026-08-14-desktop-profile-host.zh.md)
