...
The shipped apps moved their flags into their bundles: `dsh-web-app` owns the Web family, `dsh-desktop-app` owns `--help` with no bind flags, and `dsh-headless` owns the task positional and rejects a missing task as a usage error. `apps/cli/src/web.ts` is gone; `runProfile` no longer knows any flag-target row id. Out of tree, turtle-ui gained `--resume <session>` / `--session <id>` the same way, which is the design's real validation: an installed plugin added a flag with no launcher change.
