...
无 HTTP 的组装不绑定监听器。[Electron 壳](./2026-08-14-desktop-electron-shell.md) 在存在父进程 IPC 通道时，通过 `desktop-app/ipc-host` 在 Node 子进程中实例化 `HostIpcGateway`。安装包属于 [P4](../process/2026-08-14-desktop-installer-packaging.md)。web profile 仍然挂载 HTTP。
...
`packages/bundle/desktop-app/tests/composition.spec.ts` 组合 base 与 desktop-app，断言客户端名录与 api-gateway 存在，且不含 `dsh-host-webserver`、`directory-picker-auto`、HMR 或端口表达式。`tests/startup.spec.ts` 证明 `--help` 与未知 `--port` 不会提供 `desktopStartup`。`tests/desktop-app.spec.ts` 证明运行时标记与可选的 harness-source 段落，且不打印 URL。
