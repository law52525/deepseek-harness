...
The [GUI layering note](./2026-07-19-gui-layering-and-rpc-protocol.md) already reserves an Electron client that reuses `packages/client/*` and does not reuse `dsh-host-webserver`. The JSON IPC carrier lives in `dsh-host-apiproxy` ([P0](./2026-08-14-desktop-ipc-carrier.md)). The Python SDK [single-exe pipeline](./2026-07-10-single-file-executable-sdk-runtime-distribution.md) packages a Node runtime closure for linux-x64, linux-arm64, and macos-arm64; it is a headless JSON-RPC peer, not a GUI shell, and Windows is a non-goal there.

## Decision

DeepSeek Harness ships as a desktop application whose window renders the existing Web UI and whose Host is a real Node process. Installers are a macOS arm64 `.dmg` and a Windows x64 NSIS `.exe`.

### Process model

The first ship uses **two processes**:

1. **Electron shell** — `BrowserWindow`, preload, IPC to the renderer, auto-update from GitHub Releases ([P5](../process/2026-08-14-desktop-installer-release-ci.md)).
2. **Node Host child** — boots a `desktop` profile over `dsh-base`, with no HTTP listener.
...
GitHub Releases carry a notarized macOS arm64 `.dmg` and an explicitly unsigned Windows x64 `.exe`. No phase adds a user-visible HTTP URL as the desktop carrier.
...
Two-process lifetime (crash of the child, orphan windows, shutdown ordering) is new relative to `dsh web`. P2 disposes the Host before quitting.

The packaged plugin closure will drift from `dsh-web-app` if the desktop deploy manifest is edited by hand. P4 generates and verifies that closure the same way `verify-runtime-closure` gates the Python exe.

Developer preview still breaks compatibility. Auto-update from GitHub Releases replaces the whole app so extraResources is never half-written.
