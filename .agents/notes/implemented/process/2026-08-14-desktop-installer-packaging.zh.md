...
extraResources 复制整个 `apps/desktop/stage/`。electron-builder 的 `createFilter` 总会丢掉拷贝根目录下的 `node_modules`，因此 `from: stage/host` 会漏掉 Host 闭包。打包后，安装包脚本要求未打包 extraResources 里存在 `host/node_modules/@deepseek-ai/dsh/lib/bin.js` 与 `frontend/index.html`。脚本以 `node` 加上 JavaScript 入口运行 `tsx`、`electron-builder` 和 pnpm（`tsx/dist/cli.mjs`、`electron-builder/cli.js`、`npm_execpath`）；Windows 不能直接 spawn `.cmd` 垫片（CVE-2024-27980），而 `shell: true` 会把参数用空格拼接且不转义（DEP0190）。Host 的 `desktop-app` 引用 `packages/client/ui-theme/tsconfig.host.json`，这样 `tsc -b tsconfig.host.json` 能类型检查主题 settings，而不会编译该包的 Client 半边——那一半会 import Host tsdown 尚未生成的 `/remote` 模块。
...
未签名的 `pnpm run dist:desktop` 传入 electron-builder identity `-`（ad-hoc）。Windows 保持 `signAndEditExecutable: true`，以便 rcedit 把 Harness 图标写入 exe 和快捷方式；Authenticode 仍不运行，因为未签名打包会设置 `CSC_IDENTITY_AUTO_DISCOVERY=false`。NSIS 是带向导的安装程序（`oneClick: false`，`allowToChangeInstallationDirectory: true`）。[P5](./2026-08-14-desktop-installer-release-ci.md) 公证 arm64 `.dmg`（Developer ID，外加嵌套 Host 的 `node` 与 `node-spawn-helper`）并发布到 GitHub Releases；Windows 保持未签名（不做 Authenticode）。暂存树是桌面部署根目录加上复制的 Node，不是仓库根目录；被 gitignore 的 `.env` 不是 extraResource。
...
`scripts/runtime-closure.spec.ts` 与 `scripts/stage-runtime-closure.spec.ts` 钉住缺失 peer 失败、符号链接物化、deploy 之后还原 pnpm workspace state，以及 pnpm/包 bin 的 spawn 为 `process.execPath` 加上 JavaScript 入口（从不是 `.cmd` 垫片）。`apps/desktop/tests/packaged-paths.spec.ts` 钉住 extraResources 解析。`apps/desktop/tests/packager-icon.spec.ts` 钉住 `resources/icon.png` 为 1024 PNG、electron-builder 的 `icon` 字段、`signAndEditExecutable: true`，以及 NSIS 的 `oneClick: false`。`apps/desktop/tests/host-child.spec.ts` 钉住 Host 孵化 `windowsHide: true`。`scripts/desktop-shell-bundle.spec.ts` 钉住残留的 `dsh-desktop-app` / `electron-updater` import 会使打包失败。`scripts/build-desktop-installer.spec.ts` 钉住宿主操作系统目标、`pnpm run` 转发 `--`、未打包 extraResources 路径，以及工作区 `tsx` / `electron-builder` 的 JS bin。安装包脚本在 electron-builder 之前对暂存树冒烟 `--help` 与 `--dump-config`，随后要求未打包 extraResources 里有 Host bin 与前端 `index.html`。[P5](./2026-08-14-desktop-installer-release-ci.md) 在 Desktop Release 工作流上打包已签名 macOS 与未签名 Windows；打包后的 Electron GUI 启动仍是具名缺口。
