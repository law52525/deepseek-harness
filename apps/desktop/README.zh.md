...
desktop profile 的 Electron 壳。main 用系统 Node 孵化带 stdio IPC 的 `dsh --profile desktop` 子进程，在 `dsh://app/` 提供 [`dsh-web-frontend`](../web/package.json) dist，从该 Host 子进程应答 `GET`/`HEAD` `/api/session.export`，并转发不透明 RPC。[`HostIpcGateway`](../../packages/host/apiproxy/README.zh.md) 跑在子进程里；本包不解码 RPC body。渲染进程安装与 web Host 相同的 ModuleLoader facade 和 parser-preload，再通过 `BootSeams.loadBundle` 复用 Web 壳内核。
...
- **原生 OS 对话框仍在 Node 子进程** — `host.pickDirectory` 打开 Node 侧的 `osascript` / Zenity / `IFileOpenDialog` 选择器；`host.openPath` 使用现有 opener。Electron `dialog.showOpenDialog` 与 WebView 预览属于后续阶段。Host 孵化使用 `windowsHide: true`，以免 Windows 打开 `node.exe` 控制台；Win32 选择器 worker 本身已经使用 `windowsHide: true`。测试用与 web e2e 拦截 `host.openPath` 相同的方式拦截这些命令，不启动 Electron 或真实应用。
