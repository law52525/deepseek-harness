...
本参考定义 profile 启动、web 与 desktop 别名、插件管理和配置 dump 等命令模式。argv 由 [`src/args.ts`](../src/args.ts) 统一解析一次，[`src/bin.ts`](../src/bin.ts) 只会动态导入选中的运行器。
...
`dsh desktop` 是 `--profile desktop` 的硬编码别名。desktop Host 使用与 web 相同的客户端名录，且不监听 HTTP。`--help` 打印该应用的帮助且不启动服务器。
