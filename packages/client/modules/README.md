...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, and hashes the built bundle into the boot graph. When a `webServer` is present it also serves each bundle with its source map under `/plugins` and taps index.html to inject `window.__DSH_BOOT__`. Without HTTP the same graph is available through `ctx.clientModules.graph()`. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
