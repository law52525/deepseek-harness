 ...
 * in `./client/manifest.ts`), and provides `clientModules` from that graph.
 * When a `webServer` is present it also serves `/plugins/<id>/client.js` and
 * its source map and taps the index render to inject the boot manifest. HTTP
 * registration is optional so a desktop Host can read {@link ClientModuleRegistry.graph}
 * without listening.
 *
