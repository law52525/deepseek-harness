...
- **应用会等待完整名册**：只要一个 entry 失败，不依赖框架的启动页就会保留并逐项报告；不支持部分 UI 可用。激活后缺少 `uiRenderer` 同样立即失败，而不是无限等待。
