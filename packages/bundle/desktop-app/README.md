...
- **No HTTP listener** — this Host does not mount `dsh-host-webserver` or print a URL; the Electron renderer consumes `ctx.clientModules.graph()` through process IPC (`desktop-app/ipc-host`). Unary RPC, including Typert remotes such as `commands/list`, uses Connection's shared `/api` fetch on that same `HostIpcGateway`. Session-log download uses the same IPC: the shell's `dsh:` handler forwards `GET`/`HEAD` `/api/session.export` as a control document, and GET ZIP bytes return as a Host-written temp path.
...
- **Client roster must stay aligned with web-app** — a new `dsh.client` row belongs in both bundles until they share a fragment that omits HTTP rows. Omitting `@deepseek-ai/dsh-client-ui-renderer` fails the boot page because AppWebEntry requires `uiRenderer` after activation.
